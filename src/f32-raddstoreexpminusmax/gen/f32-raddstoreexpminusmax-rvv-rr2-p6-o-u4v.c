...
void xnn_f32_raddstoreexpminusmax_ukernel__rvv_rr2_p6_o_u4v(
    size_t batch,
    const float* input,
    const float* max,
    float* output,
    ...
    const void* params)
{
  ...
  if (avl > 0) {
    vfloat32m4_t vx0 = __riscv_vle32_v_f32m4(input, vl);
    input += vl;
    avl -= vl;

    do {
      vfloat32m4_t vx_proc = __riscv_vfsub_vf_f32m4(vx0, max_val, vl);
      vx_proc = __riscv_vfmax_vf_f32m4(vx_proc, xmin, vl);

      vfloat32m4_t vx1;
      size_t next_vl = 0;
      if (avl > 0) {
        next_vl = __riscv_vsetvl_e32m4(avl);
        vx1 = __riscv_vle32_v_f32m4(input, next_vl);
        input += next_vl;
      }

      vfloat32m4_t v = __riscv_vfmul_vf_f32m4(vx_proc, r_ln2f, vl);

      vint32m4_t q = __riscv_vfcvt_x_f_v_i32m4(v, vl);
      vfloat32m4_t z = __riscv_vfcvt_f_x_v_f32m4(q, vl);

      vfloat32m4_t s = __riscv_vfnmsac_vf_f32m4(vx_proc, l2uf, z, vl);
      s = __riscv_vfnmsac_vf_f32m4(s, l2lf, z, vl);

      vfloat32m4_t y = __riscv_vfmv_v_f_f32m4(c5, vl);
      y = __riscv_vfmacc_vf_f32m4(y, c6, s, vl);
      y = __riscv_vfmadd_vv_f32m4(y, s, __riscv_vfmv_v_f_f32m4(c4, vl), vl);
      y = __riscv_vfmadd_vv_f32m4(y, s, __riscv_vfmv_v_f_f32m4(c3, vl), vl);
      y = __riscv_vfmadd_vv_f32m4(y, s, __riscv_vfmv_v_f_f32m4(c2, vl), vl);
      vfloat32m4_t one = __riscv_vfmv_v_f_f32m4(1.0f, vl);
      y = __riscv_vfmadd_vv_f32m4(y, s, one, vl);
      y = __riscv_vfmadd_vv_f32m4(y, s, one, vl);

      vint32m4_t qw = __riscv_vadd_vx_i32m4(q, bias, vl);
